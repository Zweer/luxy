(function exampleCode() {
	...
	example.initCurrentExample = function(brfManager, resolution) {
		...
		//
		// Detected faces (blue) get merged (yellow) if
		// + they are roughly placed in the same location,
		// + they have roughly the same size and
		// + they have at least minMergeNeighbors of other rectangle in the same spot (4 in this case).
	...
	function printSize(rects, printAlwaysMinMax) {
		...
		if(maxWidth > 0) {
			...
			dom.updateHeadline("BRFv4 - basic - face detection - detect small faces\n" +
